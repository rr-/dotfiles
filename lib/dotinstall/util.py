...
def concat_files(sources, target, comment_prefix=None):
    for i, _ in enumerate(sources):
        sources[i] = os.path.abspath(os.path.expanduser(sources[i]))
    target = os.path.expanduser(target)
    _remove_symlink(target)
    create_dir(os.path.dirname(target))
    print('Concatenating %s to %s...' % (sources, target))
    with open(target, 'w') as ft:
        if comment_prefix:
            ft.write('%sDO NOT EDIT THIS FILE%s'
                     % (comment_prefix, os.linesep))
            ft.write('%sThis file was concatenated by dotfiles utility.%s'
                     % (comment_prefix, os.linesep))
            ft.write('%sAll changes in this file will not be retained.%s%s'
                     % (comment_prefix, os.linesep, os.linesep))
        for source in sources:
            with open(source) as fs:
                if comment_prefix:
                    ft.write('%sConcatenated file %s of %s from: %s%s%s'
                             % (comment_prefix, sources.index(source)+1,
                                len(sources), source, os.linesep, os.linesep))
                ft.write(fs.read())
                ft.write(os.linesep)
