...
class NetworkUsageProvider(object):
    ...
    def __init__(self, main_window):
        ...
        try:
            ...
            for interface in glob.glob('/sys/class/net/*'):
                ...
                if state.lower() == 'up':
                    self._rx_path = os.path.join(interface, 'statistics', 'rx_bytes')
                    self._tx_path = os.path.join(interface, 'statistics', 'tx_bytes')
    ...
    def render(self):
        if self.network_enabled:
            self._net_in_label.setText('\U0001f847 %03.0f KB/s' % (self.net_in / 1024.0))
            self._net_out_label.setText('\U0001f845 %03.0f KB/s' % (self.net_out / 1024.0))
